...
function send_sms($sms_messages, $url, $user, $pass){
   ...
   if (is_array($sms_messages) && !empty($sms_messages)) {
      foreach ($sms_messages as $sms) {
         ...
         db_update("INSERT INTO log_sms SET log_time=NOW(), account_id='$sms->account_id', callid='$sms->schedule_id', schedule_id='$sms->schedule_id', did='$sms->sms_from', client_number='$sms->sms_to', sms_text='$sms->sms_text', status='R', ob_status='$sendStatus', direction='O', bill='$bill', num_parts='$num_parts', rate='$rate'");
...
function schedule_process() {
	...
	$schedule = array();
    $scheduleList = db_select_array("SELECT ss.*, ssc.phone, ssc.group_id, ssc.id as ssc_id FROM sms_schedule as ss LEFT JOIN sms_schedule_contact as ssc ON ss.id = 
	ssc.schedule_id WHERE start_time < '$now' AND ss.status='P' AND ssc.status='P' LIMIT 10", 1);
    if(!empty($scheduleList)){
      foreach($scheduleList as $item){
         ...
         if(!empty($item->phone)){
            ...
            $groupContactList = db_select_array("SELECT id,phone FROM contacts JOIN contact_group ON contacts.id = contact_group.contact_id WHERE contact_group.group_id='$item->group_id' and contact_group.account_id='$item->account_id' LIMIT 1000", 1);
