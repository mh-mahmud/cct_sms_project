...
function send_sms($sms_messages, $url, $user, $pass){
   ...
   if (is_array($sms_messages) && !empty($sms_messages)) {
      foreach ($sms_messages as $sms) {
         ...
         //if ($num_parts > 0) db_update("INSERT INTO log_sms SET delivery_time=NOW(), account_id='$sms->account_id', callid='$sms->schedule_id', schedule_id='$sms->schedule_id', did='$sms->sms_from', client_number='$sms->sms_to', sms_text='$sms->sms_text', status='R', ob_status='$sendStatus', direction='O', bill='$bill', num_parts='$num_parts', rate='$rate'");
		 
		 if ($num_parts > 0) db_update("UPDATE log_sms SET delivery_time=NOW(), account_id='$sms->account_id', callid='$sms->schedule_id', schedule_id='$sms->schedule_id', did='$sms->sms_from', client_number='$sms->sms_to', sms_text='$sms->sms_text', status='D', ob_status='$sendStatus', direction='O', bill='$bill', num_parts='$num_parts', rate='$rate' WHERE id='$sms->id' ");
		 
         db_update("DELETE FROM sms_queue WHERE id='$sms->id' LIMIT 1");
		 
		 //db_update("UPDATE log_schedule_contact SET delivery_time=NOW(), status='D' WHERE account_id='$sms->account_id' AND schedule_id='$sms->schedule_id' AND phone='$sms->sms_to'");
		 echo "#############################  MAKE LOG  ###################################";		 		
