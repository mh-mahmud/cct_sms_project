...
class LoginController extends Controller
{
    ...
    */
    ...
    protected function sendFailedLoginResponse(Request $request)
    {
        dd("This is another hello world message");
    ...
    {        
        ...
        // hard coded some data
        $request->account_id = "600207";
        ...
        ->leftJoin($pbxDbName.'.account_profile as ap','user.account_id','ap.account_id')
        //->where(['user.account_id' => $request->account_id,'user.extn' => $request->extn,'user.status' => 'A'])
        ->where(['user.account_id' => $request->account_id,'user.active' => 'Y', 'user.allow_sms' => 'Y'])
        ->first();


        /*$user->new_user_id = "bubu";
        $user->user_type = "papunii";
...
/*dd($user);
$kaka = Hash::make("123456");
if( Hash::check("123456", $kaka) ) {
    dd("Sob thik ase");
}
dd($kaka);
dd(Hash::check($request->newPassword));

        // check user in the UserNew model
        $chk_user = UserNew::where('email', 'razu@genuitysystems.com')->first();
        if(empty($chk_user)) {
            return false;
        }*/
        ...
        if (true) {            
            $user->password = '';
            $did = Did::where('account_id','=',$request->account_id)->first();
            if($did){
                $user->did = $did->did;
            }            
            $userData = array('account_id'=>$user->account_id,'userid'=>$user->userid,'department_id'=>$user->department_id,'fname'=>$user->fname,'lname'=>$user->lname,'cname'=>$user->cname);
